...
int16_t findMinSpeed(uint8_t axis){
  ...
  setTiltSpeed(0,0);
  delayms(250);
  currentpos = (int16_t)readPos(axis); //read axis pos
  velocity -= 10;
  //fine adjustment
  while (abs((int16_t)readPos(axis)-currentpos) < 5) { //axis moved?
	delayms(50);
    if (axis == PAN){setPanSpeed(velocity, 0);} //set speed
    if (axis == TILT){setTiltSpeed(velocity, 0);}
    velocity += 1; //increase speed for next loop
...
uint8_t intToHexChar(char* buffer, int32_t input){ //turns 32 bit into to hex string
  uint8_t index = 8;
  uint8_t temp = 0;
  if (buffer != NULL){ //if valid buffer
    buffer[8] = '\n'; //last char is newline
    while (index != 0) { //iterate from end to start of buffer
      index--;
      buffer[index] = nibbleToHex(input & 0xF); //insert hex character of nibble
      input = input >> 4; //shift one nibble
    }
  }
  return 0;
}
...
int32_t getCommand(){
  ...
  switch (USART0_BUFF[0]){
    ...
    case 'F':
      if (axis == PAN){
		  return ((int32_t)(panPID.kp)<<24) + ((int32_t)(panPID.ki)<<12)+(int32_t)(panPID.kd);
	  }
	  if (axis == TILT){
		  return ((int32_t)(tiltPID.kp)<<24) + ((int32_t)(tiltPID.ki)<<12)+(int32_t)(tiltPID.kd);
	  }
      break;
    case 'L':
      PORTA.OUTTGL = LEDPIN;
